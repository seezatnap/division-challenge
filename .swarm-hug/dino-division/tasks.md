...
- [ ] (#20) Add eager dino image prefetching and a loading-state reward reveal: when the player is on problem N within a set of 5 (i.e. approaching the next reward milestone), check whether the next dinosaur's image already exists on the server; if not, kick off Gemini generation in the background so the image is likely ready by the time the reward is earned. If the player earns the reward while generation is still in flight, show a dino-themed loading/hatching animation in the reward modal and gallery slot, then poll the server (or use a streaming response) until the image is available and reveal it in place. Ensure no duplicate generation requests are made if the image already exists on disk. [5 pts] (blocked by #9)
