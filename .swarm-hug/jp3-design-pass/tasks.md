...
- [B] (#18) Add workspace contrast and typography visual tests: verify workspace text elements are light-colored on green background with sufficient contrast as shown in the comp, verify heading elements use serif font and body uses sans-serif via computed style checks [5 pts] (blocked by #8, #16) (B)
...
## Follow-up tasks (from sprint review)
- [C] (#27) Resolve the unresolved merge conflict markers in `src/app/page.tsx` (lines 1838–1848) — task #24 was marked complete but conflict markers remain; accept the `IslaSornaToolbar` stashed changes and remove the conflict markers (blocked by #23)
