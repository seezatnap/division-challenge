...
- [C] (#5) Update the motif overlay pseudo-element classes (`.motif-canopy`, `.motif-claw`, `.motif-fossil`, `.motif-track`) to work with the new green panel backgrounds, adjusting opacity and colors so they remain subtle decorative accents [5 pts] (blocked by #4) (B)
...
- [B] (#7) Wire session stats (problems solved, current streak, difficulty level) into the toolbar bar so they display as readout-style data alongside the icon buttons, maintaining the comp's dark metallic aesthetic [5 pts] (blocked by #6) (B)
