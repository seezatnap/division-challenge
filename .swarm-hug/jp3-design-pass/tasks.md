...
- [ ] (#23) Rename `tests/jp3-design-visual.test.mjs` to a visual-only target (for example `tests/jp3-design-visual.visual.mjs`) and update `npm run test:visual` to run it, so `npm test` does not unexpectedly require Playwright/browser artifacts.
